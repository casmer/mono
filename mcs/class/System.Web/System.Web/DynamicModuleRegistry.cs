    ...
    internal sealed class DynamicModuleRegistry {
        ...
        public void Add(Type moduleType) {
            ...
            if (!typeof(IHttpModule).IsAssignableFrom(moduleType)) {
				string message = String.Format(CultureInfo.CurrentCulture, SR.GetString("Type {0} is not IHttpModule"), moduleType);
            ...
            lock (_lockObj) {
                if (_entriesReadonly) {
                    ...
                    throw new InvalidOperationException(SR.GetString("Modules Already Initialized"));
                }
